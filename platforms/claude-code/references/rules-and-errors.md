...
| `entity-instance-unknown` | The note belongs in an entity folder such as `40-Projects`, but which project owns it is not inferable from the note | Leave it in the Inbox and ask the user which project it belongs to. Never drop it at the entity folder's root, and never guess an instance directory from body text — a wrong guess files it into another project, where it reads as that project's history |
| `draft-incomplete` | The note declares itself unfinished — it carries the Vault's draft tag, or its body still holds an unreplaced `{{placeholder}}` | Leave it in the Inbox; that is where an unfinished capture belongs. Filing is what makes a note read as settled, so report the signals in `draft_signals` and say what completes it: finish the content and clear the marker. Never strip the tag to make it filable — the marker is the user's statement about their own note |
