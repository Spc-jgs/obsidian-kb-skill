...
- Two archives of the same note collided. Uniqueness was enforced inside one month's folder, but a note links its archive by bare stem and Obsidian resolves that vault-wide, so archiving the same note in a later month produced two `<name>·原文.md` under different months and made the note's own link ambiguous — a `defect` the write helper created and its own audit then reported. The suffix search now covers the whole archive tree.
- `--replace` dropped the archive it was documented to keep. Both the refusal message and `rules-and-errors.md` say to pass it only when the user wants to keep both, but it overwrote the single `source_archive` key: the earlier archive stayed on disk still pointing back at the note, while the note no longer pointed at it. A second archive is now added rather than substituted — one stays a plain string, several become a list.
- A captured source that was not UTF-8 crashed with a traceback instead of the JSON envelope every other failure in `archive-source` uses. The caller parses stdout, so this surfaced as an unexplained tool failure with no code to act on, on exactly the input the feature exists for. Both `--stdin` and `--content-file` now refuse with `undecodable-source-content`, and the reference says to re-capture rather than transcode: a guessed encoding is not byte-faithful evidence.
- Date filters accepted a date that does not exist. Note dates were matched with an ISO-*shaped* pattern while the filter flags themselves were validated as real calendar dates, so `2026-13-45` passed and was then range-compared as text, sorting a thirteenth month as if it were a date. A note's own metadata now clears the same bar its filters do.
