...
一个仓库包含两个职责分离的 Skill：`obsidian-knowledge-retrieval` 负责只读搜索、引用和问答；`obsidian-knowledge-base` 仅在用户明确授权后创建、更新和治理笔记。当前稳定版本为 **v1.32.0**。
...
| 本地 helper | `search-vault`、`review-projects`、`vault-info`、`doctor` | 创建、更新、分类、索引、链接、审计等 |

检索使用确定性的本地词法排序，不需要 embedding 模型、向量数据库、常驻服务或联网索引。标题、别名、标签、标题层级、wikilink 和正文按不同权重参与排序。中英混写的 Vault 还会经过一层双语概念词表扩展：中文提问也能命中英文笔记，扩展词降权参与排序，并在结果里如实标注哪些词是词表加的。详见[只读检索](docs/retrieval.md)。
...
| 项目复苏雷达 | 用可解释的只读队列找回受阻或逐渐失温的项目 | [只读检索](docs/retrieval.md) |
