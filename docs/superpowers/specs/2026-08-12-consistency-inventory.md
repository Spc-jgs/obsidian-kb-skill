...
| 46 | A truncated run's `mean_soft_score` ↔ the summary declaring it was truncated | `test_the_summary_names_the_case_that_ended_a_truncated_run` and `test_stopping_early_names_the_case_so_a_partial_mean_is_not_read_as_whole` — the 2026-08-18 baseline stopped after 15 of 36 runs and still reported `mean_soft_score` under the key a complete run uses, so a number covering 5 of 12 cases read exactly like one covering all 12; `planned_runs` and `completed_runs` differed but named no case, and the mean is what reports quote |
...
| 49 | `disconnected-note`'s exempt types ↔ `review-captures`'s `CAPTURE_TYPES` | `test_a_type_exempted_from_connectivity_is_still_watched_somewhere` — the audit stopped reporting web-clips on the argument that `review-captures` already reported 20 of the 23 reference-Vault findings by a stronger question; drop `web-clip` from `CAPTURE_TYPES` and neither reports it, so the note goes unwatched and the argument for the exemption expires with nothing saying so |
...
| 53 | Every Vault-walking helper's exclusion list ↔ the backup directory | `test_every_helper_that_walks_the_vault_skips_the_backup_directory` — three modules keep their own list on purpose, because they answer different questions, but `.obsidian-kb-backups/` is not a question any of them should differ on: `review_captures.IGNORED_DIRECTORIES` omitted it and counted three backed-up web-clips as captures, reporting two of them as never reopened. A copy is never reopened by definition, so it can only drag the rate down — web-clip's revisit rate read 0.226 instead of 0.275 |
| 54 | The adversarial corpus's mean note length ↔ the reference Vault's | `test_the_corpus_mean_length_stays_near_the_reference_vault` — BM25 penalises length *relative to the corpus mean*, so the mean decides which notes are penalised at all. Two 76 KB notes held 97% of the set's bytes and put the mean at 8464 against the real 4247, leaving every everyday note unpenalised: `adv-dilution-06` ranked first here while its real-Vault counterpart ranked second. Restoring 120 filler paragraphs turns the guard red at 8468 |
