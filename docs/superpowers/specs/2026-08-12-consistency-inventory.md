...
| 41 | The eval fixture's `hard_failures` list ↔ the codes `score_run` can actually raise | `test_the_recorded_hard_failure_codes_match_the_ones_the_scorer_can_raise` — the fixture recorded 8 of 14 and named two the scorer never raises, so the gate read wider than it is; the unraisable pair is now kept in `hard_failures_not_implemented` with what happened to each rather than deleted |
