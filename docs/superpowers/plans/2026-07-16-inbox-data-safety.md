...
> **Status: NOT MERGED.** Tasks 1–3 accepted; Task 4 failed independent review
> and its architecture was later superseded. Tasks 5–9 never started.
> The code lives on the local branch `fix/inbox-data-safety`.
> Review evidence: `docs/superpowers/sdd/inbox-data-safety/`.
>
> Some of what this plan set out to fix shipped separately in v1.25.1
> (fail-closed frontmatter parsing, destination rollback, truthful result
> counts). Backup, source hashes, the journaled transaction, and restore
> did not.
