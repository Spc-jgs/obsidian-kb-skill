...
- Produces: `FolderIndexConfig`, `read_folder_index_config()`,
  ...
  `StaticIndexEntry`, `StaticIndexResult`, `append_static_index_entry()`.
...
Create `tests/test_folder_index_policy.py` with real Vault fixtures and exact
assertions:
...
assert result.status in {"appended", "unmanaged", "missing"}
```
...
Return `StaticIndexResult(status: str, index: Path | None)` as an immutable
result rather than printing. Keep Folder Index and Dataview listings untouched.
Do not add duplicate detection or another behavior change in this structural
branch. Reject an entry whose note path is outside the Vault or whose target
folder resolves outside the Vault by using `vault_paths`.
