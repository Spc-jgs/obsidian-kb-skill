...
## Task Memory (multi-agent handoff) — opt-in, **OFF by default**

> Disabled unless the user explicitly opts in (says "开启任务记忆" / "handoff", or a task note carries `task-memory: enabled`). When off, no task note is read or written — **zero overhead**. This layer is still experimental, so its full spec is kept *out* of this always-loaded body to save context tokens.

- **To use it:** the full spec (TASK.md structure, handoff protocol, `obsidian-update-note`) lives in `references/task-memory.md` next to this file. **Only read that file when the user actually turns task memory on** — do not load it otherwise.
- **To turn off:** set `task-memory: disabled` (or delete the task note); honor the global master switch `OBSIDIAN_KB_TASK_MEMORY=off`.
